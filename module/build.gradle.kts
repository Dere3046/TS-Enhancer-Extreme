...
val installWebUITask = tasks.register<Exec>("installWebUI") {
    group = "module"
    workingDir(rootProject.file("webui"))
    executable("npm")
    args("install")
    onlyIf { !rootProject.file("webui/node_modules").exists() }
}

val buildWebUITask = tasks.register<Exec>("buildWebUI") {
    group = "module"
    dependsOn(installWebUITask)
    workingDir(rootProject.file("webui"))
    executable("npm")
    args("run", "build")

    inputs.dir(rootProject.file("webui/src"))
    inputs.file(rootProject.file("webui/package.json"))
    inputs.file(rootProject.file("webui/vite.config.ts"))
    outputs.dir(rootProject.file("webui/dist"))
}
