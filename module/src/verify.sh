...
# Verify all files in the zip (excluding .sha256 files themselves)
print_cn "- 正在校验文件完整性..."
print_en "- Verifying file integrity..."

for file in $(unzip -l "$ZIPFILE" | awk 'NR>3 {print $4}' | grep -v '\.sha256$' | grep -v '/$' | grep -v '^$'); do
  case "$file" in
    META-INF/com/google/android/update-binary) continue ;;
    META-INF/com/google/android/updater-script) continue ;;
  esac
  extract "$ZIPFILE" "$file" "$TMPDIR_FOR_VERIFY" -q
done

print_cn "- 所有文件校验通过"
print_en "- All files verified"
