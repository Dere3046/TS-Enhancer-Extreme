...
# Only extract files for the detected architecture + common files
FILES="
script/*
webroot/*
post-fs-data.sh
...
print_cn "- 检测设备架构"
print_en "- Detecting device architecture"
...
case $ARCH in
    ...
    x86|i686)
        ...
        ;;
    *)
        abort_cn "不支持的架构: $ARCH"
        abort_en "Unsupported architecture: $ARCH"
        ;;
esac
print_cn "- 架构: $ARCH_DIR"
print_en "- Architecture: $ARCH_DIR"
##END##
...
print_cn "- 提取架构二进制文件"
print_en "- Extracting architecture binaries"
for bin in tseed cmd_tool; do
    extract "$ZIPFILE" "bin/$ARCH_DIR/$bin" "$MODPATH"
    mv -f "$MODPATH/bin/$ARCH_DIR/$bin" "$MODPATH/bin/$bin"
done
rmdir "$MODPATH/bin/$ARCH_DIR" 2>/dev/null

# Extract architecture-specific library
extract "$ZIPFILE" "lib/$ARCH_DIR/libverify.so" "$MODPATH"
mv -f "$MODPATH/lib/$ARCH_DIR/libverify.so" "$MODPATH/lib/libverify.so"
rmdir "$MODPATH/lib/$ARCH_DIR" 2>/dev/null

# Select architecture-specific signature files
print_cn "- 选择架构签名文件"
print_en "- Selecting architecture signature files"
[ -f "$MODPATH/Ranav.$ARCH_DIR" ] && mv -f "$MODPATH/Ranav.$ARCH_DIR" "$MODPATH/Ranav"
[ -f "$MODPATH/Ranavs.$ARCH_DIR" ] && mv -f "$MODPATH/Ranavs.$ARCH_DIR" "$MODPATH/Ranavs"
rm -f "$MODPATH"/Ranav.* "$MODPATH"/Ranavs.*
...
# Give execute permission to all binaries and scripts
chmod -R +x "$MODPATH/bin/"
chmod -R +x "$MODPATH/script/"
